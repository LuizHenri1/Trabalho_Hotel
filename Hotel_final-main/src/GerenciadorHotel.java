...
public class GerenciadorHotel {
    ...
    public void cadastrarHospede(Hospede hospede) throws HotelException {
        try {
            ...
            if (hospedes.stream().anyMatch(h -> h.getCpf().equals(hospede.getCpf()))) {
                throw new HotelException("Já existe um hóspede cadastrado com este CPF");
            }
            
            hospedes.add(hospede);
            System.out.println("Debug: Hóspede cadastrado com sucesso. CPF: " + hospede.getCpf());
            System.out.println("Debug: Total de hóspedes: " + hospedes.size());
            
        } catch (ValidacaoException e) {
            throw new HotelException("Erro ao cadastrar hóspede: " + e.getMessage());
        }
    }
    ...
    public Hospede buscarHospedePorCpf(String cpf) throws HotelException {
        System.out.println("Debug: Buscando hóspede com CPF: " + cpf);
        System.out.println("Debug: Lista de hóspedes contém " + hospedes.size() + " registros");
        
        return hospedes.stream()
                .filter(h -> {
                    boolean match = h.getCpf().equals(cpf);
                    System.out.println("Debug: Comparando CPF " + h.getCpf() + " com " + cpf + ": " + match);
                    return match;
                })
                .findFirst()
                .orElseThrow(() -> new HotelException("Hóspede não encontrado"));
    }
